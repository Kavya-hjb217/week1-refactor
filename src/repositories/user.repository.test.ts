...
describe("UserRepository", () => {
  ...
  describe("create", () => {
    it("should create a valid user with a generated UUID", () => {
      //1st test case
    ...
    it("should return InvalidEmail error for malformed emails", () => {
      //2nd test case
      const result = repo.create({
        name: "John",
        email: "not-an-email",
    ...
    it("should return DuplicateEmail error if email is already registered", () => {
      //3rd test case
  ...
  describe("findById", () => {
    it("should find a user by their UUID", () => {
      //4th test case
      const creation = repo.create({
        name: "Alice",
    ...
    it("should return UserNotFound for a non-existent UUID", () => {
      //5th test case
  ...
  describe("update", () => {
    it("should update user name and refresh updatedAt timestamp", async () => {
      //6th test case
      const creation = repo.create({
        name: "Original",
        email: "test@test.com",
  ...
  describe("delete", () => {
    it("should remove a user and return success", () => {
      //7th test case
      const creation = repo.create({
        name: "To Delete",
        email: "del@test.com",
    ...
    it("should return UserNotFound when deleting a non-existent user", () => {
      //8th test case
  ...
  describe("UserRepository negative test cases", () => {
    ...
    it('should return a "UserNotFound" error when updating a non-existent ID', () => {
      // ARRANGE: Define an ID that definitely doesn't exist in the Map
      const fakeId = "999-invalid-id";
      const updateData = { name: "Ghost User" };

      // ACT: Try to update that non-existent user
      const result = repo.update(fakeId, updateData);

      // ASSERT: Check that it failed exactly how we expected
      expect(result.success).toBe(false);

      // This confirms your "Result Pattern" is working
      if (!result.success) {
        expect(result.error).toBe("UserNotFound");
      }
    });
